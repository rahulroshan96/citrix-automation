...
class auditnslogaction(base_resource) :
    ...
    def serverdomainname(self) :
        """Auditserver name as a FQDN. Mutually exclusive with serverIP.<br/>Minimum length =  1<br/>Maximum length =  255."""
        try :
    ...
    def domainresolveretry(self) :
        """Time, in seconds, for which the NetScaler appliance waits before sending another DNS query to resolve the host name of the audit server if the last query failed.<br/>Default value: 5<br/>Minimum length =  5<br/>Maximum length =  20939."""
        try :
